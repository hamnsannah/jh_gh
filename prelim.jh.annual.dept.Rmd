...
g <- ggplot(data = supplier.agg.10, aes(x = Supp.By.Year, fill = factor(Year))) + 
  ...
  scale_y_continuous(labels = scales::dollar) +
  scale_fill_brewer(direction = 1, palette = "YlGnBu", name = "Year") + theme_dark() + 
...
data.both.item$Item.By.Year <- paste(data.both.item$Description, 
                                           data.both.item$Year)
item.agg <- aggregate(Total.Sales ~ Item.By.Year + Description + Year, data.both.item, sum)
cy <- year(freemium.end.date)
py <- year(freemium.end.date)-1
...
item.merge.py <- item.agg.py[,c(2,4)]
...
item.merge <- mutate(item.merge, "Growth" = Total.Sales - `Prior Yr`, "Perc.Growth" = 
  ...
  arrange(desc(Total.Sales)) %>%
  select(1,4:8)
colnames(item.merge)[c(2,4)] <- c("Sales Current Yr", "Sales Prior Yr")
...
item.agg.units.merge <- merge(item.agg.cy.pretty, item.agg.units, by.x = "Description", by.y = "Description", all.x = TRUE, all.y = TRUE)
